...
public class Info {
	...
	// "alliframe.height=document.body.clientHeight+";
	...
	public static HashMap getUser(HttpServletRequest request) {
		HashMap map = (HashMap) (request.getSession().getAttribute("admin") == null
	...
	public static String getselect(String name, String tablename, String zdname) {
		...
		/*
		 * for(HashMap permap:new
		 * CommDAO().select("select * from "+tablename+" order by id desc")){
		 * select+="<option value=\""+permap.get(zdname)+"\">"+permap.get(zdname)+
	...
	public static String getselect(String name, String tablename, String zdname, String where) {
		...
		/*
		 * for(HashMap permap:new CommDAO().select("select * from "+tablename+" where "
		 * +where+" order by id desc")){ String optionstr = "";
		 * 
		 ...
		 * 
		 * 
		 * }else{ for(String str:zdname.split(";")) { String zdstr = str.split("~")[0];
		 * String zdnamestr = str.split("~")[1].equals("无名")?"":(str.split("~")[1]+":");
		 * optionstr+=zdnamestr+permap.get(zdstr)+" - "; } }
		 * 
		 * 
		 * 
		 * if(optionstr.indexOf(" - ")>-1)optionstr=optionstr.substring(0,optionstr.
		 * length()-3);
	...
	public static String getradio(String name, String tablename, String zdname, String where) {
		...
		/*
		 * for(HashMap permap:new CommDAO().select("select * from "+tablename+" where  "
		 * +where+" order by id desc")){ String check="";
		 * if(dxii==0)check="checked=checked";
		 * 
		 * String optionstr = ""; for(String str:zdname.split(";")) { String zdstr =
		 * str.split("~")[0]; String zdnamestr =
		 * str.split("~")[1].equals("无名")?"":(str.split("~")[1]+":");
		 * optionstr+=zdnamestr+permap.get(zdstr)+" - "; }
		 * if(optionstr.length()>0)optionstr=optionstr.substring(0,optionstr.length()-3)
		 * ;
		 * 
		 * radio+="<label><input type='radio' name='"+name+"' "+check+" value=\""
	...
	public static void writeExcel(String fileName, String prosstr, java.util.List<List> plist,
		...
		/*
		 * List<List> mlist = new
		 * CommDAO().selectforlist("select "+cols+" from "+fileName+" "
		...
		if (wwb != null) {
			 ...
			 * try {
			 * 
			 * Iterator it = t.iterator(); int jj=0; while(it.hasNext()) { Label label1 =
			 * new Label(jj, i,"");
			 * 
			 * String a = it.next().toString(); label1.setString(a); ws.addCell(label1);
			 * jj++; }
			 * 
			 * i++; } catch (RowsExceededException e1) { // TODO Auto-generated catch block
			 * e1.printStackTrace(); } catch (WriteException e1) { // TODO Auto-generated
			 * catch block e1.printStackTrace(); } }
			 * 
			 * try { //从内存中写入文件中 wwb.write(); //关闭资源，释放内存 wwb.close(); } catch (IOException
			 * e) { e.printStackTrace(); } catch (Exception e) { e.printStackTrace(); } }
			 * 
			 * try { response.sendRedirect("/pianotrainwebstie/upload?filename="+fileName.
			 * substring(fileName.lastIndexOf("/")+1)); } catch (IOException e) {
			 * e.printStackTrace();
			 */
		}
	...
	public static String getcheckbox(String name, String tablename, String zdname, String where) {
		 ...
		 * ;
		 * 
		 * checkbox+="<label><input type='checkbox' name='"+name+"' value=\""+optionstr+
		 * "\">"+optionstr+"</label>\n"; }
	...
	public static String getcheckboxDk(String name, String tablename, String zdname, String nstr, String where) {
		 ...
		 * optionstr+=zdnamestr+permap.get(zdstr)+" - "; }
		 * if(optionstr.length()>0)optionstr=optionstr.substring(0,optionstr.length()-3)
		 * ; String nbs = ""; if(i>0)nbs="&nbsp;";
		 * checkbox+="<label>"+nbs+"<input type='checkbox' name='"+name+"' value=\""
		 * +optionstr+"\">"+optionstr+"</label>&nbsp;&nbsp;\n";
		 * checkbox+="<label>&nbsp;- "+nstr+" &nbsp;<input type='text' size='5' name='"
		 * +optionstr+"' value=\"\"></label><br />\n"; i++; }
		 * checkbox+="<input type=hidden name='"
		 * +name+"' value='' /><input type=hidden name='dk-"+name+"-value' value='"
		 * +nstr+"' />";
		 */
		return checkbox;
	}
				...
				if (!str1.equals("content")) {
					...
					if (str2.indexOf(" - ") > -1) {
						for (String strch : str2.split(" ~ ")) {
							...
							jscode += " {\n";
							jscode += " document.getElementsByName('" + strchname + "')[0].value='"
					...
					jscode += "{\n";
					jscode += "document.getElementById(\"txt2\").src=\"/pianotrainwebstie/upfile/"
							+ map.get("filename2") + "\";\n";
					jscode += "}\n";

					jscode += "if(document.getElementById(\"txt3\"))\n";
					jscode += "{\n";
					jscode += "document.getElementById(\"txt3\").src=\"/pianotrainwebstie/upfile/"
							+ map.get("filename3") + "\";\n";
					jscode += "}\n";

					jscode += "if(document.getElementById(\"txt4\"))\n";
					jscode += "{\n";
					jscode += "document.getElementById(\"txt4\").src=\"/pianotrainwebstie/upfile/"
							+ map.get("filename4") + "\";\n";
					jscode += "}\n";

					jscode += "if(document.getElementById(\"txt5\"))\n";
					jscode += "{\n";
					jscode += "document.getElementById(\"txt5\").src=\"/pianotrainwebstie/upfile/"
							+ map.get("filename5") + "\";\n";
					jscode += "}\n";
	...
	public static String getImgUpInfo(int height) {
		String jscode = "";
		jscode += "<img style=\"cursor: hand\" onclick=\"uploadimg()\" src=\"/pianotrainwebstie/js/nopic.jpg\" id=txt height=\""
				+ height + "\"/>";
	...
	public static String getImgUpInfo2(int height) {
		String jscode = "";
		jscode += "<img style=\"cursor: hand\" onclick=\"uploadimg2()\" src=\"/pianotrainwebstie/js/nopic.jpg\" id=txt2 height=\""
				+ height + "\"/>";
	...
	public static String getImgUpInfo3(int height) {
		String jscode = "";
		jscode += "<img style=\"cursor: hand\" onclick=\"uploadimg3()\" src=\"/pianotrainwebstie/js/nopic.jpg\" id=txt3 height=\""
				+ height + "\"/>";
	...
	public static String getImgUpInfo4(int height) {
		String jscode = "";
		jscode += "<img style=\"cursor: hand\" onclick=\"uploadimg4()\" src=\"/pianotrainwebstie/js/nopic.jpg\" id=txt4 height=\""
				+ height + "\"/>";
	...
	public static String getImgUpInfo5(int height) {
		String jscode = "";
		jscode += "<img style=\"cursor: hand\" onclick=\"uploadimg5()\" src=\"/pianotrainwebstie/js/nopic.jpg\" id=txt5 height=\""
				+ height + "\"/>";
