$(function(){
	...
	$('.banner').each(function(){
		...
		var dianzishops=e.attr("data-dianzishop");
		var dianzishops_s=e.attr("data-small");
		var dianzishops_m=e.attr("data-middle");
		var dianzishops_b=e.attr("data-big");
		var num=e.find(".carousel .dianzishop").length;
		var win=$(window).width();
		var i=1;

		if(interval==null){interval=5};
		if(dianzishops==null || dianzishops<1){dianzishops=1};
		if(dianzishops_s!=null && win>760){dianzishops=dianzishops_s};
		if(dianzishops_m!=null && win>1000){dianzishops=dianzishops_m};
		if(dianzishops_b!=null && win>1200){dianzishops=dianzishops_b};

		var dianzishopWidth=Math.ceil(e.outerWidth()/dianzishops);
		var page=Math.ceil(num/dianzishops);
		e.find(".carousel .dianzishop").css("width",dianzishopWidth+ "px");
		e.find(".carousel").css("width",dianzishopWidth*num + "px");
		
		var carousel=function(){
			i++;
			if(i>page){i=1;}
			$showbanner(e,i,dianzishops,num);
		};
		...
		if(pointer!=0 && page>1){
			...
			pager.find("li").click(function(){
				$showbanner(e,$(this).val(),dianzishops,num);
			});
	...
	$showbanner=function(e,i,dianzishops,num){
		var after=0,leftx=0;
		leftx = - Math.ceil(e.outerWidth()/dianzishops)*(dianzishops)*(i-1);
		if(i*dianzishops > num){after=i*dianzishops-num;leftx= - Math.ceil(e.outerWidth()/dianzishops)*(num-dianzishops);};
